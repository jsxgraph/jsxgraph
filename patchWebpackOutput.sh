...
sed -i "s/\(return __webpack_exports__\);/\1.default;/" distrib/jsxgraphsrc.js    # Works in AMD
                                                                                  # but conflicts in node together
                                                                                  # with above line

# Do not require canvas outside of node.
# AMD
sed -i "s/\[\"canvas\"\], factory/[], factory/" distrib/jsxgraphsrc.js
# Replace all occurences of factory[require("canvas")] and put it back in the first string
# sed -i "s/factory(require(\"canvas\"));/factory();/g" distrib/jsxgraphsrc.js
# Browser
