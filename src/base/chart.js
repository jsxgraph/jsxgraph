...
define([
      ...
      drawPie: function (board, y, attributes) {
        ...
        for (i = 0; i < y.length; i++) {
          ...
          attributes.fillcolor =
            colorArray && colorArray[i % colorArray.length];
          attributes.labelcolor =
            colorArray && colorArray[i % colorArray.length];
          attributes.highlightfillcolor =
            highlightColorArray &&
            highlightColorArray[i % highlightColorArray.length];

          sector[i] = board.create(
            "sector",
            [center, p[i], p[i + 1]],
            attributes
          );
      ...
      if (Type.exists(table)) {
        ...
        for (i = 0; i < len; i++) {
          ...
          if (hStrokeColor && hStrokeColor.length === len) {
            ...
            attr.highlightstrokecolor = Color.hsv2rgb(
              ((i + 1) / len) * 360,
              0.9,
              1.0
            );
          ...
          if (hFillColor && hFillColor.length === len) {
            ...
            attr.highlightfillcolor = Color.hsv2rgb(
              ((i + 1) / len) * 360,
              0.9,
              0.6
            );
  ...
  JXG.Legend.prototype.drawVerticalLegend = function (board, attributes) {
    ...
    for (i = 0; i < this.label_array.length; i++) {
      ...
      this.myAtts.highlightstrokecolor =
        this.color_array[i % this.color_array.length];
      this.myAtts.label = {
        offset: [10, 0],
