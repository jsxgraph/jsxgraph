...
JXG.Circle = function (board, method, par1, par2, attributes) {
  ...
  this.constructor(
    board,
    attributes,
    Const.OBJECT_TYPE_CIRCLE,
    Const.OBJECT_CLASS_CIRCLE
  );
...
JXG.extend(
  ...
  /** @lends JXG.Circle.prototype */ {
    ...
    X: function (t) {
      return (
        this.Radius() * Math.cos(t * 2 * Math.PI) +
        this.center.coords.usrCoords[1]
      );
    },
    ...
    Y: function (t) {
      return (
        this.Radius() * Math.sin(t * 2 * Math.PI) +
        this.center.coords.usrCoords[2]
      );
    },
...
JXG.createCircle = function (board, parents, attributes) {
  ...
  for (i = 0; i < parents.length; i++) {
    if (Type.isPointType(board, parents[i])) {
      p = p.concat(
        Type.providePoints(board, [parents[i]], attributes, "circle", [
          point_style[i],
        ])
      );
  ...
  if (p.length === 2 && Type.isPoint(p[0]) && Type.isPoint(p[1])) {
    ...
    p[0].elementClass === Const.OBJECT_CLASS_CIRCLE &&
    Type.isPoint(p[1])
  ) {
    // Circle/Point
    el = new JXG.Circle(board, "pointCircle", p[1], p[0], attr);
  } else if (
    p[1].elementClass === Const.OBJECT_CLASS_CIRCLE &&
    Type.isPoint(p[0])
  ) {
    // Point/Circle
    el = new JXG.Circle(board, "pointCircle", p[0], p[1], attr);
  } else if (
    p[0].elementClass === Const.OBJECT_CLASS_LINE &&
    Type.isPoint(p[1])
  ) {
    // Line/Point
    el = new JXG.Circle(board, "pointLine", p[1], p[0], attr);
  } else if (
    p[1].elementClass === Const.OBJECT_CLASS_LINE &&
    Type.isPoint(p[0])
  ) {
    // Point/Line
