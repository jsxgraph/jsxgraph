...
define([
  ...
  JXG.extend(
    ...
    /** @lends JXG.Line.prototype */ {
      ...
      hasPoint: function (x, y) {
        ...
        vnew = new Coords(Const.COORDS_BY_SCREEN, vnew.slice(1), this.board)
          .usrCoords;
          ...
          if (d > Mat.eps) {
            if ((d1 > d2 && drag2) || (d1 <= d2 && drag2 && !drag1)) {
              this.point2.setPositionDirectly(Const.COORDS_BY_USER, [
                this.point1.X() +
                  ((this.point2.X() - this.point1.X()) * dnew) / d,
                this.point1.Y() +
                  ((this.point2.Y() - this.point1.Y()) * dnew) / d,
              ]);
              this.point2.fullUpdate();
            } else if ((d1 <= d2 && drag1) || (d1 > d2 && drag1 && !drag2)) {
              this.point1.setPositionDirectly(Const.COORDS_BY_USER, [
                this.point2.X() +
                  ((this.point1.X() - this.point2.X()) * dnew) / d,
                this.point2.Y() +
                  ((this.point1.Y() - this.point2.Y()) * dnew) / d,
              ]);
      ...
      updateRenderer: function () {
        ...
        if (this.visPropCalc.visible) {
          ...
          this.isReal =
            ...
            Mat.innerProduct(this.stdform, this.stdform, 3) >=
              Mat.eps * Mat.eps;
        ...
        if (Type.evaluate(this.visProp.snaptogrid)) {
          if (this.parents.length < 3) {
            ...
            if (
              ...
              sX =
                ticks.ticksDelta *
                (Type.evaluate(ticks.visProp.minorticks) + 1);
            }
            if (
              sY <= 0 &&
              this.board.defaultAxes &&
            ...
            if (sX > 0 && sY > 0) {
              ...
              /*
                        ...
                        */
              c2 = Geometry.projectPointToLine(
                { coords: c1 },
                this,
                this.board
              );
  ...
  JXG.createAxis = function (board, parents, attributes) {
    ...
    if (
      ...
      // Create ticks
      attr_ticks = Type.copyAttributes(
        attributes,
        board.options,
        "axis",
        "ticks"
