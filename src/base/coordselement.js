...
define([
    ...
    /** @lends JXG.CoordsElement.prototype */ {
      ...
      updateGliderFromParent: function () {
        ...
        if (slide.elementClass === Const.OBJECT_CLASS_CIRCLE) {
          ...
          // In case, the point is a constrained glider.
          this.updateConstraint();
          c = Geometry.projectPointToTurtle(this, slide, this.board)[0]
            .usrCoords;
        } else if (slide.elementClass === Const.OBJECT_CLASS_CURVE) {
          ...
          if (
            ...
            c = [
              slide.center.X() +
                r * Math.cos(this.position * delta + baseangle),
              slide.center.Y() +
                r * Math.sin(this.position * delta + baseangle),
            ];
          } else {
            // In case, the point is a constrained glider.
            this.updateConstraint();
            ...
            if (isTransformed) {
              ...
              c = Geometry.projectPointToCurve(this, slide, this.board)[0]
                .usrCoords;
            }
          }
        } else if (Type.isPoint(slide)) {
        ...
        for (i = 0; i < len; i++) {
          ...
          if (Type.exists(el) && el !== this) {
            if (Type.isPoint(el)) {
              ...
              if (
                !Type.evaluate(el.visProp.straightfirst) &&
                projection[1] < 0.0
              ) {
              ...
              projCoords = Geometry.projectPointToTurtle(
                this,
                el,
                this.board
              )[0];
      ...
      setPositionDirectly: function (method, coords) {
        ...
        for (i = this.transformations.length - 1; i >= 0; i--) {
          ...
          this.initialCoords.setCoordinates(
            Const.COORDS_BY_USER,
            Mat.matVecMult(
              Mat.inverse(this.transformations[i].matrix),
              newCoords
            )
          );
        }
      ...
      moveAlong: function (path, time, options) {
        ...
        if (Type.isArray(path)) {
          ...
          if (!Type.exists(options.interpolate) || options.interpolate) {
            ...
            for (i = 0; i < steps; i++) {
              interpath[i] = [];
              interpath[i][0] = neville[0](
                ((steps - i) / steps) * neville[3]()
              );
              interpath[i][1] = neville[1](
                ((steps - i) / steps) * neville[3]()
              );
            }
          } else {
            ...
            for (i = 0; i < steps; ++i) {
              ...
              interpath[i] = [];
              interpath[i][0] =
                (1.0 - part) * p[pos].X() + part * p[pos + 1].X();
              interpath[i][1] =
                (1.0 - part) * p[pos].Y() + part * p[pos + 1].Y();
            }
      ...
      _anim: function (direction, stepCount) {
        ...
        if (this.slideObject.elementClass === Const.OBJECT_CLASS_LINE) {
          ...
          dX = Math.round(
            ((sp2c[1] - sp1c[1]) * this.intervalCount) / stepCount
          );
          dY = Math.round(
            ((sp2c[2] - sp1c[2]) * this.intervalCount) / stepCount
          );
          if (direction > 0) {
          ...
          res = Geometry.projectPointToCurve(
            this,
            this.slideObject,
            this.board
          );
          ...
          this.slideObject.elementClass === Const.OBJECT_CLASS_CIRCLE
        ) {
          alpha = 2 * Math.PI;
          if (direction < 0) {
   ...
   */
  JXG.CoordsElement.create = function (
    Callback,
    board,
    coords,
    attr,
    ...
    if (!isConstrained) {
      if (Type.isNumber(coords[0]) && Type.isNumber(coords[1])) {
        ...
        Type.isObject(coords[0]) &&
        Type.isTransformationOrArray(coords[1])
      ) {
        // Transformation
