...
JXG.extend(
  ...
  /** @lends JXG.Ticks.prototype */ {
    ...
    setTicksSizeVariables: function (pos) {
      ...
      // Grid-like ticks?
      this.minStyle =
        Type.evaluate(this.visProp.minorheight) < 0 ? "infinite" : "finite";
      this.majStyle =
        Type.evaluate(this.visProp.majorheight) < 0 ? "infinite" : "finite";
    },
    ...
    processTickPosition: function (
      ...
      isLabelPosition =
        Math.round(tickPosition / ticksDelta) % ticksPerLabel === 0;
          ...
          for (i = 0; i <= full; i += delta) {
            x.push(
              this.board.origin.scrCoords[1] +
                r * Math.cos(i) * this.board.unitX
            );
            y.push(
              this.board.origin.scrCoords[2] +
                r * Math.sin(i) * this.board.unitY
            );
          }
          return [x, y, major];
        }
    ...
    updateRendererLabels: function () {
      ...
      for (j = lenData; j < lenLabels; j++) {
        ...
        this.labels[j].visProp.visible = this.labels[
          j
