...
define([
  ...
  JXG.Board = function (
     ...
     * @type Object
     */
    this.containerObj = Env.isBrowser
      ? this.document.getElementById(this.container)
      : null;
    ...
    if (
      ...
      id !== "" &&
      Env.isBrowser &&
      !Type.exists(this.document.getElementById(id))
    ) {
      this.id = id;
    } else {
  ...
  JXG.extend(
    ...
    /** @lends JXG.Board.prototype */ {
      ...
      checkFrameRate: function (evt) {
        ...
        if (
          !handleEvt &&
          (time - this.touchMoveLast) * this.attr.maxframerate >= 1000
        ) {
          handleEvt = true;
        }
        if (handleEvt) {
      ...
      getCoordsTopLeftCorner: function () {
        var cPos,
          ...
          docElement =
            ownerDoc.documentElement || this.document.body.parentNode,
      ...
      moveObject: function (x, y, o, evt, type) {
        ...
        this.drag_position = Statistics.add(
          this.drag_position,
          this._drag_offset
        );
        //
      ...
      twoFingerTouchObject: function (tar, drag, id) {
        ...
        if (
          ...
          fix = new Coords(
            Const.COORDS_BY_SCREEN,
            [fixEl.Xprev, fixEl.Yprev],
            this
          ).usrCoords;
          // Previous finger position
          op = new Coords(
            Const.COORDS_BY_SCREEN,
            [moveEl.Xprev, moveEl.Yprev],
            this
          ).usrCoords;
          // New finger position
          np = new Coords(Const.COORDS_BY_SCREEN, [moveEl.X, moveEl.Y], this)
            .usrCoords;
      ...
      addKeyboardEventHandlers: function () {
        if (
          this.attr.keyboard.enabled &&
          !this.hasKeyboardHandlers &&
          Env.isBrowser
        ) {
          Env.addEvent(
            this.containerObj,
      ...
      pointerDownListener: function (evt, object) {
        ...
        if (
          ...
          Type.isFunction(this.document.selection.empty)
        ) {
      ...
      pointerMoveListener: function (evt) {
        ...
        //}
        this.triggerEventHandlers(
          ["touchmove", "move", "pointermove", "MSPointerMove"],
          [evt, this.mode]
        );
        this.updateQuality = this.BOARD_QUALITY_HIGH;
        ...
        if (this.selectingMode) {
          ...
          for (i = 0; i < tmpTouches.length; i++) {
            ...
            if (Type.isPoint(tmpTouches[i].obj)) {
              ...
              tmpTouches[i].obj.elementClass === Const.OBJECT_CLASS_LINE
            ) {
              found =
                (touchTargets[0] && touchTargets[0].found) ||
                (touchTargets[1] && touchTargets[1].found);
            } else if (
              tmpTouches[i].obj.elementClass === Const.OBJECT_CLASS_CIRCLE
            ) {
      ...
      updateContainerDims: function () {
        ...
        if (window && window.getComputedStyle) {
          ...
          w -=
            parseFloat(css.getPropertyValue("border-left-width")) +
            parseFloat(css.getPropertyValue("border-right-width"));
          h -=
            parseFloat(css.getPropertyValue("border-top-width")) +
      ...
      updateInfobox: function (el) {
        ...
        if (
          (!Type.evaluate(this.attr.showinfobox) && vpsi === "inherit") ||
          !vpsi
        ) {
          return this;
        }
        ...
        try {
          ...
          for (el in object.childElements) {
            if (object.childElements.hasOwnProperty(el)) {
              object.childElements[el].board.removeObject(
                object.childElements[el]
              );
            }
          }
      ...
      showXML: function () {
        ...
        f.document.open();
        f.document.write(
          "<pre>" + Type.escapeHTML(this.xmlString) + "<" + "/pre>"
        );
        f.document.close();
        return this;
      },
      ...
      updateElements: function (drag) {
        ...
        for (el = 0; el < this.objectsList.length; el++) {
          ...
          pEl
            .update(!Type.exists(drag) || pEl.id !== drag.id)
            .updateVisibility();
        }
      ...
      create: function (elementType, parents, attributes) {
        ...
        if (Type.isFunction(JXG.elements[elementType])) {
          ...
          throw new Error(
            "JSXGraph: create: Unknown element type given: " + elementType
          );
        }
      ...
      setBoundingBox: function (bbox, keepaspectratio, setZoom) {
        ...
        this.moveOrigin(
          -this.unitX * (bbox[0] - offX),
          this.unitY * (bbox[1] + offY)
        );
      ...
      animate: function () {
        ...
        for (el in this.animationObjects) {
          if (
            ...
            if (
              !Type.exists(o.animationData) &&
              !Type.exists(o.animationPath)
            ) {
              this.animationObjects[el] = null;
              delete this.animationObjects[el];
      ...
      updateCSSTransforms: function () {
        ...
        while (o) {
          ...
          o =
            o.parentNode === o.getRootNode() ? o.parentNode.host : o.parentNode;
        }
      ...
      _createSelectionPolygon: function (attr) {
        ...
        if (!Type.exists(this.selectionPolygon)) {
          selectionattr = Type.copyAttributes(
            attr,
            Options,
            "board",
            "selection"
        ...
        var brd = this,
          Roulette = function () {
            ...
            this.rolling = function () {
              ...
              hp = new Complex(
                Numerics.D(c1.X)(t1_new),
                Numerics.D(c1.Y)(t1_new)
              );
              gp = new Complex(
                Numerics.D(c2.X)(t2_new),
                Numerics.D(c2.Y)(t2_new)
              );
