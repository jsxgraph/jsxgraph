...
JXG.Polygon = function (board, vertices, attributes) {
  this.constructor(
    board,
    attributes,
    Const.OBJECT_TYPE_POLYGON,
    Const.OBJECT_CLASS_AREA
  );
    ...
    removePoints: function (p) {
      ...
      if (this.withLines) {
        for (i = 0; i < partition.length; i++) {
          ...
          if (
            partition[i][1] !== 0 &&
            partition[i][0] !== nvertices.length - 1
          ) {
...
JXG.createPolygon = function (board, parents, attributes) {
  ...
  if (
    ...
    attr_points = Type.copyAttributes(
      attributes,
      board.options,
      "polygon",
      "vertices"
    ...
    for (i = 0; i < le; i++) {
      ...
      points.push(
        board.create("point", [obj.vertices[i], parents[1]], attr_points)
      );
    }
  } else {
    points = Type.providePoints(board, parents, attributes, "polygon", [
      "vertices",
    ]);
    if (points === false) {
      throw new Error(
