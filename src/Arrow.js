 ...
 * @param {Board} board Reference to the board the arrow is drawn on.
 * @param {Point} p1 Origin of the arrow.
 * @param {Point} p2 Defines the direction of the arrow.
...
JXG.createArrow = function(board, parentArr, atts) {
    var el;
    
    if ( (JXG.IsPoint(parentArr[0])) && (JXG.IsPoint(parentArr[1])) ) {
        el = new JXG.Arrow(board, parentArr[0], parentArr[1], atts['id'], atts['name']);
    } // Ansonsten eine fette Exception um die Ohren hauen
    else
        throw ("Can't create arrow with parent types '" + (typeof parentArr[0]) + "' and '" + (typeof parentArr[1]) + "'.");
