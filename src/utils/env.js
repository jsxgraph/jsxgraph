  ...
  JXG.extend(
    ...
    /** @lends JXG */ {
      ...
      supportsCanvas: function () {
        ...
        return (
          hasCanvas ||
          (this.isBrowser && !!document.createElement("canvas").getContext)
        );
      },
      ...
      isWebkitApple: function () {
        return (
          this.isApple() &&
          navigator.userAgent.search(/Mobile\/[0-9A-Za-z.]*Safari/) > -1
        );
      },
      ...
      getDimensions: function (elementId, doc) {
        ...
        if (display !== "none" && display !== null) {
          ...
          style = window.getComputedStyle
            ...
            : element.style;
          return {
            width: pixelDimRegExp.test(style.width)
              ? parseFloat(style.width)
              : 0,
            height: pixelDimRegExp.test(style.height)
      ...
      removeEvent: function (obj, type, fn, owner) {
        ...
        if (i === -1) {
          JXG.debug(
            "removeEvent: no such event function in internal list: " + fn
          );
          return;
        }
        ...
        try {
          ...
          JXG.debug(
            "event not registered in browser: (" + type + " -- " + fn + ")"
          );
        }
        ...
        if (owner["x_internal" + type]) {
          ...
          for (i = len - 1; i >= 0; i--) {
            JXG.removeEvent(
              obj,
              type,
              owner["x_internal" + type][i].origin,
              owner
            );
          }
      ...
      scaleJSXGraphDiv: function (wrap_id, inner_id, doc) {
        ...
        for (i = 0; i < len_pseudo; i++) {
          try {
            rule =
              "#" + wrap_id + pseudo_keys[i] + " #" + inner_id + rule_inner_l;
