...
define([
  ...
  JXG.createEllipse = function (board, parents, attributes) {
    var polarForm,
      ...
      // focus 1 and focus 2
      F = [],
      attr_foci = Type.copyAttributes(
        attributes,
        board.options,
        "conic",
        "foci"
      ),
      attr_center = Type.copyAttributes(
        attributes,
        board.options,
        "conic",
        "center"
    ...
    curve.hasPoint = function (x, y) {
      ...
      if (Type.evaluate(this.visProp.hasinnerpoints)) {
        ...
        dist =
          p.distance(Const.COORDS_BY_USER, ac) +
          p.distance(Const.COORDS_BY_USER, bc);
  ...
  JXG.createParabola = function (board, parents, attributes) {
    ...
    curve.X = function (phi, suspendUpdate) {
      ...
      det =
        (B[1] - A[1]) * (M[2] - A[2]) - (B[2] - A[2]) * (M[1] - A[1]) >= 0
          ? 1
          : -1;
  ...
  JXG.createConic = function (board, parents, attributes) {
    var polarForm,
      ...
      points = [],
      p = [],
      attr_point = Type.copyAttributes(
        attributes,
        board.options,
        "conic",
        "point"
    ...
    polarForm = function (phi, suspendUpdate) {
      ...
      if (!suspendUpdate) {
        if (givenByPoints) {
          ...
          c1 = degconic(
            Mat.crossProduct(p[0], p[1]),
            Mat.crossProduct(p[2], p[3])
          );
          c2 = degconic(
            Mat.crossProduct(p[0], p[2]),
            Mat.crossProduct(p[1], p[3])
          );
      ...
      if (eigen[0][1][1] <= 0.0 && eigen[0][2][2] <= 0.0) {
        v = Mat.matVecMult(rotationMatrix, [
          1 / c,
          Math.cos(phi) / a,
          Math.sin(phi) / b,
        ]);
