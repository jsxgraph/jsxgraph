...
JXG.createSector = function (board, parents, attributes) {
  ...
  if (
    ...
    (Type.isNumber(parents[4]) ||
      Type.isFunction(parents[4]) ||
      Type.isString(parents[4]))
  ) {
    type = "2lines";
  } else {
    points = Type.providePoints(
      board,
      parents,
      attributes,
      "sector",
  ...
  if (type === "2lines") {
    ...
    if (Type.isArray(parents[2])) {
      ...
      el.direction1 =
        Mat.innerProduct(
          v,
          [0, el.line1.stdform[2], -el.line1.stdform[1]],
          3
        ) >= 0
          ? +1
          : -1;
    ...
    if (Type.isArray(parents[3])) {
      ...
      el.direction2 =
        Mat.innerProduct(
          v,
          [0, el.line2.stdform[2], -el.line2.stdform[1]],
          3
        ) >= 0
          ? +1
          : -1;
  ...
  el.getLabelAnchor = function () {
    ...
    coords = new Coords(
      ...
      [
        pmc[1] +
          Math.cos(angle * 0.5) * bxminusax -
          Math.sin(angle * 0.5) * byminusay,
        pmc[2] +
          Math.sin(angle * 0.5) * bxminusax +
          Math.cos(angle * 0.5) * byminusay,
      ],
      this.board
    );
    ...
    l_vp.position = Geometry.calcLabelQuadrant(
      Geometry.rad([1, 0], [0, 0], vec)
    );
...
JXG.createAngle = function (board, parents, attributes) {
  ...
  el.updateDataArray = function () {
    ...
    if (
      Math.abs(deg - 90.0) <
      Type.evaluate(this.visProp.orthosensitivity) + Mat.eps
    ) {
    ...
    if (
      ...
      (type !== "sectordot" && this.dot.visProp.visible)
    ) {
