...
define([
  ...
  Mat.Plot = {
    ...
    _plotRecursive_v2: function (curve, a, ta, b, tb, depth, delta) {
      ...
      pnt.setCoordinates(
        Const.COORDS_BY_USER,
        [curve.X(tc, true), curve.Y(tc, true)],
        false
      );
      c = pnt.scrCoords;
      ...
      isCusp =
        depth < this.smoothLevel + 2 &&
        ds[0] < cusp_threshold * (ds[1] + ds[2]);
    ...
    _insertLimesPoint: function (curve, pnt, t, depth, limes) {
      ...
      if (
        ...
        (Math.abs(this._lastUsrCrds[2]) === Infinity &&
          Math.abs(limes.left_y) === Infinity)
      ) {
    ...
    _findStartPoint: function (curve, a, ta, b, tb) {
      ...
      pnt.setCoordinates(
        Const.COORDS_BY_USER,
        [curve.X(ta, true), curve.Y(ta, true)],
        false
      );
    ...
    thiele: function (t, recip, t_values, idx, degree) {
      ...
      for (i = degree; i > 1; i--) {
        v =
          (t - t_values[idx + i]) /
          (recip[i][idx + 1] - recip[i - 2][idx + 1] + v);
      }
      return (
        recip[0][idx + 1] + (t - t_values[idx + 1]) / (recip[1][idx + 1] + v)
      );
    },
      ...
      for (idx = 0; idx < components.length; idx++) {
        ...
        for (g = 0; g <= groups.length; g++) {
          ...
          if (g < groups.length) {
            ...
            i = groups[g].idx;
            if (
              groups[g].type === "borderleft" ||
              groups[g].type === "borderright"
            ) {
              this.handleBorder(curve, comp, groups[g], x_table, y_table);
            } else {
              this._seconditeration_v4(
                curve,
                comp,
                groups[g],
                x_table,
