...
define([
  ...
  Mat.Clip = {
    ...
    _addToList: function (list, coords, pos) {
      ...
      if (
        len > 0 &&
        Math.abs(list[len - 1].coords.usrCoords[0] - coords.usrCoords[0]) <
          eps &&
        Math.abs(list[len - 1].coords.usrCoords[1] - coords.usrCoords[1]) <
          eps &&
    ...
    _addVertex: function (path, vertex, DEBUG) {
      if (
        !isNaN(vertex.coords.usrCoords[1]) &&
        !isNaN(vertex.coords.usrCoords[2])
      ) {
        path.push(vertex);
      }
      ...
      if (
        ...
        for (i = 0; i < len; i++) {
          if (Type.exists(obj[i].coords)) {
            ...
            this._addToList(
              S,
              new Coords(Const.COORDS_BY_USER, obj[i], board),
              i
            );
    ...
    greinerHormann: function (subject, clip, clip_type, board) {
      ...
      if (
        len > 0 &&
        Geometry.distance(
          S[0].coords.usrCoords,
          S[len - 1].coords.usrCoords,
          3
        ) < Mat.eps
      ) {
        S.pop();
