...
define(["jxg", "math/math", "utils/type"], function (JXG, Mat, Type) {
  ...
  JXG.extend(MatInterval.prototype, {
    ...
    bounded: function (lo, hi) {
      return this.set(
        Mat.IntervalArithmetic.prev(lo),
        Mat.IntervalArithmetic.next(hi)
      );
    },
    ...
    open: function (lo, hi) {
      return this.assign(
        Mat.IntervalArithmetic.next(lo),
        Mat.IntervalArithmetic.prev(hi)
      );
    },
    ...
    pow: function (x, power) {
      ...
      if (power % 1 === 0) {
        ...
        if (x.lo < 0) {
          // [negative, positive]
          if ((power & 1) === 1) {
            return new MatInterval(
              -this.powLo(-x.lo, power),
              this.powHi(x.hi, power)
            );
          }
