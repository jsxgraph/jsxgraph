...
define(["utils/type", "math/math"], function (Type, Mat) {
  ...
  Mat.Metapost = {
        ...
        if (k === 0) {
          ...
          if (s.ltype === this.MP_END_CYCLE || s.ltype === this.MP_OPEN) {
            ...
            if (s.ltype === this.MP_CURL) {
              ...
              theta[n] =
                -(vv[n - 1] * ff) / (this.FRACTION_ONE - ff * uu[n - 1]);
              break;
            }
            if (s.ltype === this.MP_GIVEN) {
              theta[n] =
                s.left_given() - Math.atan2(delta_y[n - 1], delta_x[n - 1]);
        ...
        if ((st >= 0 && sf >= 0) || (st <= 0 && sf <= 0)) {
          ...
          if (sine > 0) {
            ...
            if (p.right_tension() < 0) {
              if (
                this.mp_ab_vs_cd(Math.abs(sf), this.FRACTION_ONE, rr, sine) < 0
              ) {
                rr = Math.abs(sf) / sine;
              }
            }
            if (q.left_tension() < 0) {
              if (
                this.mp_ab_vs_cd(Math.abs(st), this.FRACTION_ONE, ss, sine) < 0
              ) {
                ss = Math.abs(st) / sine;
              }
            }
    ...
    curve: function (point_list, controls) {
      ...
      knots = this.makeknots(
        point_list,
        Type.evaluate(controls.tension),
        controls.isClosed
      );
