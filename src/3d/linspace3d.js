...
define([
  ...
  JXG.createLine3D = function (board, parents, attributes) {
    ...
    point = Type.providePoints3D(view, [parents[1]], attributes, "line3d", [
      "point",
    ])[0];
    ...
    if (
      ...
      point1 = point;
      point2 = Type.providePoints3D(view, [parents[2]], attributes, "line3d", [
        "point2",
      ])[0];
      direction = function () {
        return [
    ...
    el.element2D = view.create(
      "segment",
      [point1.element2D, point2.element2D],
      attr
    );
      ...
      updateDataArray: function () {
        ...
        if (
          ...
          for (j = 0; j < planes.length; j++) {
            ...
            p = view.intersectionPlanePlane(
              this,
              view.defaultAxes[planes[j]],
              d
            );
  ...
  JXG.createPlane3D = function (board, parents, attributes) {
    ...
    point = Type.providePoints3D(view, [parents[1]], attributes, "plane3d", [
      "point",
    ])[0];
    if (point === false) {
      // TODO Throw error
