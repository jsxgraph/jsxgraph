 ...
 */
JXG.Board.prototype.construct = function (
  string,
  mode,
  params,
  paraIn,
        ...
        if (!prop) {
          ...
          if (noMacro) {
            ...
            if (splitted[i].search(/^[\[\]].*[\[\]]$/) != -1) {
                ...
                if (defElements[j].search(/[\[\]]/) != -1) {
                  ...
                  if (typeof defElements[j] == "string") {
                    defElements[j] = (function (el, board) {
                      return function () {
                        return JXG.getReference(board, el.charAt(0)).Dist(
                          JXG.getReference(board, el.charAt(1))
                        ); // TODO
                      };
                    })(defElements[j], this);
                  } else {
              ...
              output.circles.push(
                this.create("circle", defElements, attributes)
              );
              if (objName != "") {
              ...
              defElements = [
                new Function("x", "var y = " + tmp + "; return y;"),
              ];
              attributes.name = objName;
              ...
              splitted[i].search(/#(.*)\(\s*([0-9])\s*[,|]\s*([0-9])\s*\)/) !=
              -1
            ) {
              // Text element
              defElements = []; // [0-9\.\-]+
