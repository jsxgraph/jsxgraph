...
declare namespace JXG {
  ...
  export class JessieCode {
     ...
     */
    sub(
      a: number | number[] | Point,
      b: number | number[] | Point
    ): number | number[];
    /**
  ...
  export function cloneAndCopy<S extends object, T extends object>(
    obj: S,
    obj2: T
  ): S | T;
  export function cmpArrays(a1: unknown[], a2: unknown[]): boolean;
  export function coordsArrayToMatrix(
    coords: unknown[],
    split: boolean
  ): unknown[];
  /**
   ...
   */
  export function copyAttributes<T>(
    attributes: T,
    options: JXGSettings,
    ...s: string[]
  ): T;
  ...
  export interface Numerics {
    ...
    bezier(
      points: Point[]
    ): [x: (t: number) => number[], y: (t: number) => number[]];
    bspline(
      points: Point[],
      order: number
    ): [
