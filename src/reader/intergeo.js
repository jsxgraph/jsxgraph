    ...
    /** @lends JXG.IntergeoReader.prototype */ {
      ...
      storePoint: function (node) {
        ...
        if (p.nodeName === "homogeneous_coordinates") {
          ...
          JXG.debug(
            "This coordinate type is not yet implemented: " + p.nodeName
          );
          return;
        }
      ...
      readConstraint: function (tree, s) {
        ...
        if (node.nodeName === "line_through_two_points") {
          ...
          node.nodeName === "line_perpendicular_to_line_through_point"
        ) {
          ...
          node.nodeName === "other_intersection_point_of_two_circles"
        ) {
          this.addOtherIntersectionPointOfTwoCircles(node);
        } else if (
          node.nodeName === "other_intersection_point_of_circle_and_line"
        ) {
          ...
          JXG.debug(
            "readConstraints: not implemented: " +
              node.nodeName +
              ": " +
              param[0]
          );
      ...
      addPointIntersectionOfTwoLines: function (node) {
        ...
        this.objects[param[0]] = this.board.create(
          "intersection",
          [l1, l2, 0],
          {
            name: param[0],
      ...
      addPointOnCircle: function (node) {
        ...
        this.board.create(
          "glider",
          [
            this.objects[param[0]].coords[1],
            this.objects[param[0]].coords[2],
            c,
          ],
          {
            name: param[0],
      ...
      prepareString: function (fileStr) {
        ...
        if (fileStr.indexOf("<") !== 0) {
          ...
          fileStr = new JXG.Util.Unzip(bA).unzipFile(
