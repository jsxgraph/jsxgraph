...
JXG.GeogebraReader = new function () {
     ...
     */
    this.ggbAct = function (tree, board, type, m, n) {
        var s1, s2, a,
            v1 = m,
            v2 = n;
        ...
        switch (type.toLowerCase()) {
            ...
            case 'command':
                ...
                switch(s1.toLowerCase()) {
                    case 'name':
                        return 'JXG.boards[\'' + board.id + '\'].select(\''+ s2 +'\').getName()'; //TODO korrigiere getName()
                        break;
                }
                break;
            case 'var':
                if(v2) {
                    ...
                    if(v1 == 'PI') {
                        ...
                        if(typeof board.ggb[v1] != 'undefined') {
     ...
     */
    this.ggbParse = function(tree, board, exp, element) {
        var element = (element) ? JXG.getRef(board, board.ggbElements[element].id) : false;
        ...
        function __parse( tree, board, src, err_off, err_la ) {
        ...
        var error_count = 0;
        var str = exp;
        if( ( error_count = __parse( tree, board, str, error_offsets, error_lookaheads ) ) > 0 ) {
     ...
     */
    this.coordinates = function(board, gxtEl, Data) {
        var a, labelOffset = {}, tmp;
     ...
     */
    this.checkElement = function(tree, board, name) {
// Segment[A, B] nur bis Version 2.4 ? In 2.5 schon (x(A), x(B)) und durch Parser loesbar
        ...
        // } else
        if(typeof board.ggbElements[name] == 'undefined' || board.ggbElements[name] == '') {
            var input = JXG.GeogebraReader.getElement(tree, name);
     ...
     */
    this.functionParse = function(format, type, exp) {
    ...
    this.writeBoard = function(tree, board) {
        ...
        board.origin.scrCoords = [1, parseInt(boardData.getElementsByTagName("coordSystem")[0].getAttribute("xZero"), 10), parseInt(boardData.getElementsByTagName("coordSystem")[0].getAttribute("yZero"), 10)];
        board.unitX = (boardData.getElementsByTagName("coordSystem")[0].getAttribute("scale")) ? parseInt(boardData.getElementsByTagName("coordSystem")[0].getAttribute("scale"), 10) : 1;
        board.unitY = (boardData.getElementsByTagName("coordSystem")[0].getAttribute("yscale")) ? parseInt(boardData.getElementsByTagName("coordSystem")[0].getAttribute("yscale"), 10) : board.unitX;

        board.fontSize = (tree.getElementsByTagName("gui")[0] && tree.getElementsByTagName("gui")[0].getElementsByTagName("font")[0]) ? parseInt(tree.getElementsByTagName("gui")[0].getElementsByTagName("font")[0].getAttribute("size"), 10) : 12;
        ...
        switch(gxtEl.type) {
                ...
                try {
                    ...
                    if(typeof input != 'undefined') {
                        if (ma!=null && ma.length==3) {
                            ...
                            var q = JXG.GeogebraReader.checkElement(tree, board, ma[1]);
                    ...
                    if(false && output[0].getAtribute('type') && output[0].getAttribute('type') == 'numeric') {
                        ...
                        p = board.create('text', [function(){return m.X();}, function(){return m.Y();}, function(){
                            return "<span style='text-decoration: overline'>"+ input[0].name + input[1].name +"</span> = "
                                + JXG.trimNumber(JXG.getRef(board, input[0].id).Dist(JXG.getRef(board, input[1].id)).toFixed(board.ggbProps.decimals));
                        }], attr);
                        p.Value = function () {
            ...
            case 'vector':
                ...
                try {
                    ...
                    JXG.debugWST("* <b>Err:</b> Vector " + attr.name + e +"<br>\n");
                    return false;
                }
                break;
            case 'rotate':
            ...
            case 'slope':
                ...
                try {
                    ...
                    var t = board.create('text', [function(){return m.X();}, function(){return m.Y();},
                        function(){ return "&nbsp;&nbsp;" + (slopeWidth > 1 ? slopeWidth.toString() : "") + " " + this.name + " = " + JXG.trimNumber((slopeWidth * input[0].getSlope()).toFixed(board.ggbProps.decimals)); }], attr);
                    attr.name = "";
            ...
            case 'text':
                ...
                try {
                    if(element.getElementsByTagName('isLaTeX')[0] && element.getElementsByTagName('isLaTeX')[0].getAttribute('val') == 'true') {
                        ...
                        while(rx = t.match(/(.*?)" \+ (.+) \+ "(.*)/)) {
                            var re2 = JXG.GeogebraReader.ggbParse(tree, board, RegExp.$2);
...
//    break;
    ...
    this.readGeogebra = function(tree, board) {
        ...
        board.ggbProps.decimals = parseInt(tree.getElementsByTagName('geogebra')[0].getElementsByTagName('kernel')[0].getElementsByTagName('decimals')[0].getAttribute('val'));
