  ...
  JXG.extend(
    ...
    /** @lends JXG.CinderellaReader.prototype */ {
      read: function () {
        ...
        for (i = 0; i < dataLines.length; i++) {
          ...
          if (dataLines[i].search(/FreePoint.+/) !== -1) {
            pCoords = dataLines[i].slice(
              dataLines[i].search(/FreePoint.+/) + 11
            );
            ...
            defPoints = dataLines[i].slice(
              dataLines[i].search(/CircleBy3.+/) + 10
            );
            defPoints = defPoints.split(",");
            defName = [];
            for (j = 0; j < defPoints.length; j++) {
            ...
            defPoints = dataLines[i].slice(
              dataLines[i].search(/Parallel.+/) + 11
            );
            defPoints = defPoints.split(",");
            defName = [];
      ...
      readPointProperties: function (dataLines, i) {
        ...
        objAppearance = objAppearance
          .slice(1, objAppearance.length - 1)
          .split(",");
