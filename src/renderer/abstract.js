    ...
    /** @lends JXG.AbstractRenderer.prototype */ {
      ...
      _getHighlighted: function (el) {
        ...
        if (
          ...
          !Type.exists(el.board.highlightedObjects)
        ) {
      ...
      updatePoint: function (el) {
        ...
        if (!isNaN(el.coords.scrCoords[2] + el.coords.scrCoords[1])) {
          ...
          size *=
            !el.board || !zoom
              ? 1.0
              : Math.sqrt(el.board.zoomX * el.board.zoomY);
          s1 = size === 0 ? 0 : size + 1;
      ...
      setTabindex: function (element) {
        var val;
        if (
          element.board.attr.keyboard.enabled &&
          Type.exists(element.rendNode)
        ) {
          val = Type.evaluate(element.visProp.tabindex);
          if (
            !element.visPropCalc.visible ||
            Type.evaluate(element.visProp.fixed)
          ) {
            val = null;
          }
      ...
      setDraft: function (el) {
        ...
        if (el.type === Const.OBJECT_TYPE_POLYGON) {
          ...
          this.setObjectStrokeWidth(
            el,
            el.board.options.elements.draft.strokeWidth
          );
        }
      },
          ...
          if (board.attr.shownavigation) {
            if (board.attr.showzoom) {
              ...
              createButton(
                "o",
                board.zoom100,
                board.container + "_navigation_100"
              );
              createButton(
                "+",
                board.zoomIn,
                board.container + "_navigation_in"
              );
            }
            createButton(
              "\u2190",
