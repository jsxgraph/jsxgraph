  ...
  /** @lends JXG.SVGRenderer.prototype */ {
    ...
    _createArrowHead: function (el, idAppendix, type) {
      ...
      node2.setAttributeNS(
        null,
        "stroke",
        Type.evaluate(el.visProp.strokecolor)
      );
      node2.setAttributeNS(
        null,
        "stroke-opacity",
      ...
      /*
            ...
            */
      node3 = this.container.ownerDocument.createElementNS(
        this.svgNamespace,
        "path"
      );
      h = 5;
    ...
    updateInternalText: function (el) {
      ...
      if (el.rendNode.getAttributeNS(null, "class") !== el.visProp.cssclass) {
        el.rendNode.setAttributeNS(
          null,
          "class",
          Type.evaluate(el.visProp.cssclass)
        );
    ...
    createPrim: function (type, id) {
      var node = this.container.ownerDocument.createElementNS(
        this.svgNamespace,
        type
      );
    ...
    updateGradient: function (el) {
      ...
      node2.setAttributeNS(
        null,
        "style",
        "stop-color:" + col + ";stop-opacity:" + op
      );
      node3.setAttributeNS(
        null,
        "style",
    ...
    setObjectTransition: function (el, duration) {
      ...
      if (
        ...
        transitionStr =
          " color " + duration + "ms," + " opacity " + duration + "ms";
      } else {
        transitionStr =
          " fill " +
          duration +
    ...
    dumpToDataURI: function (ignoreTexts) {
      ...
      if (this.container.hasChildNodes() && Type.exists(this.foreignObjLayer)) {
        ...
        while (svgRoot.nextSibling) {
          ...
          values = values.concat(
            this._getValuesOfDOMElements(svgRoot.nextSibling)
          );
      ...
      if (
        (svg.match(/xmlns="http:\/\/www.w3.org\/2000\/svg"/g) || []).length > 1
      ) {
      ...
      if (
        ...
        this.foreignObjLayer.hasChildNodes()
      ) {
