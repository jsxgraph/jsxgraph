...
describe("Test text handling", function() {
    ...
    it("parse", function() {
        var el = board.create('text', [0, 10, 'Hello <span class="my">world</span>'], {parse: true});
    ...
    it("parse + MathJax", function() {
        var A = board.create('point', [2, 4]),
            el = board.create('text', [0, 10, '\\[2X(A)+\\pi=<value>2X(A) + PI</value>\\]'], {parse: true, useMathJax: true});
        expect(el.rendNode.innerHTML).toEqual('\\[2X(A)+\\pi=7.14\\]');
    });

    it("text function + MathJax", function() {
        var A = board.create('point', [2, 4]),
            el = board.create('text', [0, 10, () => '\\[' + A.X().toFixed(3) + '+ \\pi\\]'], {useMathJax: true});
        expect(el.rendNode.innerHTML).toEqual('\\[2.000+ \\pi\\]');
    });

    it("button + function", function() {
        var A = board.create('point', [2, 4]),
            el = board.create('button',[-3, 3, () => 'Press ' + A.X().toFixed(3), function(){ }], {});
        expect(el.rendNodeButton.innerHTML).toEqual('Press 2.000');
    });

    it("button + MathJax + function", function() {
        var A = board.create('point', [2, 4]),
            el = board.create('button',[-3, 3, '\\(\\int\\) <value>X(A)</value>', function(){ }], {useMathJax: true});
        expect(el.rendNodeButton.innerHTML).toEqual('\\(\\int\\) 2.00');
    });

    it("input + MathJax + function", function() {
        var A = board.create('point', [2, 4]),
            el = board.create('input',[-3, 1, "test", '\\(\\int\\) <value>X(A)</value>: '], {parse: true, useMathJax: true});
        expect(el.rendNodeLabel.innerHTML).toEqual('\\(\\int\\) 2.00: ');
    });

    it("checkbox + MathJax + function", function() {
        var A = board.create('point', [2, 4]),
            el =  board.create('checkbox', [-3, 0.5, () => 'Change \\(\\int' + A.X().toFixed(4) + '\\)'], {useMathJax: true });
        expect(el.rendNodeLabel.innerHTML).toEqual('Change \\(\\int2.0000\\)');
    });

    it("text + default parse + value", function() {
        var A = board.create('point', [2, 4]),
            el =  board.create('text', [-2, -1, 'val_x X(A)+1+&pi;=<value>X(A) + 1 + PI</value>'], {});
        expect(el.rendNode.innerHTML).toEqual('val<sub>x</sub> X(A)+1+π=6.14');
    });

    it("text + default parse + value + MathJax", function() {
        var A = board.create('point', [2, 4]),
            el =  board.create('text', [-2, -1, '\\[2X(A)+\\pi=<value>2X(A) + PI</value>\\]'], {useMathJax: true});
        expect(el.rendNode.innerHTML).toEqual('\\[2X(A)+\\pi=7.14\\]');
    });

    it("text + parse=false + value", function() {
        var A = board.create('point', [2, 4]),
            el =  board.create('text', [-2, -1, '<sqrt>val_x 1</sqrt>+1+&pi;=<value>1 + 1 + PI</value>'], {parse: false});
        expect(el.rendNode.innerHTML).toEqual('<sqrt>val_x 1</sqrt>+1+π=<value>1 + 1 + PI</value>');
    });
