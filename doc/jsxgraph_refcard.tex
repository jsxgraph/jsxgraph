...
\metax{axis,grid,showNavigation,showCopyright:}{\hfill true/false}
\metax{}{show axis, grid, zoom/navigation buttons, display copyright}

{\sl Properties and methods of the board:}\par
\metax{brd.snapToGrid:true/false:}{grid mode}
\metax{brd.suspendUpdate()}{stop updating (if speed is needed)}
\metax{brd.unsuspendUpdate()}{restart updating}
\metax{brd.addChild(brd2)}{Connect board {\tt brd2} to board {\tt brd}}

\section{Basic commands}
\metax{var el = brd.create(\'type\',[parents],$\{$attributes$\}$);}{}
\metax{el.setProperty($\{$key1:value1,key2:value2,...$\}$);}{}

\section{Point}
\metax{brd.create(\'point\',[parents],$\{$attributes$\}$);}{}
...
\metax{brd.create(\'glider\',[parents],$\{$attributes$\}$);}{}
...
\section{Line}
\metax{brd.create(\'line\',[parents],$\{$attributes$\}$);}{}
...
\section{Circle}
\metax{brd.create(\'circle\',[parents],$\{$attributes$\}$);}{}
...
\section{Polygon}
\metax{brd.create(\'polygon\',[p1,p2,...],$\{$attributes$\}$);}{}
\metax{[p1,p2,...]}{The array of points}
is connected by line segments and the inner area is filled.

\metax{brd.create(\'regularpolygon\',[p1,p2,n],$\{$attributes$\}$);}{}

\section{Slider}
\metax{var s = brd.create(\'slider\',[[a,b],[c,d],[e,f,g]],$\{$atts$\}$);}{}
...
-- \metax{brd.create(\'curve\',[parents],$\{$attributes$\}$);}{}
...
-- \metax{brd.create(\'spline\',[p1,p2,...],$\{$attributes$\}$);}{}
\metax{[p1,p2,...]}{{\sl Cubic spline:} array of points}

-- \metax{brd.create(\'riemannsum\',[f,n,type],$\{$atts$\}$);}{}
{\sl Riemann sum} of type 'left', 'right', 'middle', 'trapezodial', 'upper', or 'lower'

-- \metax{brd.create(\'integral',[[a,b],f],$\{$atts$\}$);}{}
Display the area $\int_a^b f(x)dx$. 


\section{Tangent, normal}
\metax{var el = brd.create(\'tangent\',[g],$\{$attributes$\}$);}{}
\metax{var el = brd.create(\'normal\',[g],$\{$attributes$\}$);}{}
...
\metax{brd.create(\'ellipse\',[p1,p2,p3],$\{$attributes$\}$);}{}
\metax{brd.create(\'ellipse\',[p1,p2,a],$\{$attributes$\}$);}{}
\metax{brd.create(\'hyperbola\',[p1,p2,p3],$\{$attributes$\}$);}{}
\metax{brd.create(\'hyperbola\',[p1,p2,a],$\{$attributes$\}$);}{}
\par{\sl -- parabola:}\hfill defined by the focus and the directrix (line). 
\metax{brd.create(\'parabola\',[p1,line],$\{$attributes$\}$);}{}
...
(x,y,z)^\top
$$
\metax{brd.create(\'conic\',[p1,$\ldots$,p5],$\{$atts$\}$);}{}
\metax{brd.create(\'conic\',[$a_{00}$,$a_{11}$,$a_{22}$,$a_{01}$,$a_{02}$,$a_{12}$],$\{$atts$\}$);}{}

\section{Turtle}
%\metax{var t = brd.create(\'turtle\');}{}
%\metax{var t = brd.create(\'turtle\',[],$\{$attributes$\}$);}{}
\metax{var t = brd.create(\'turtle\',[parents],$\{$atts$\}$);}{}
...
\metax{el = brd.create(\'text\',[x,y,"Hello"]);}{}
\metax{el = brd.create(\'text\',[x,y,f]);}{where}
...
\metax{el = brd.create(\'image\',[uri-string,[x,y],[w,h]]);}{}
...
\metax{el = brd.create(\'circumcirclesector\',[A,B,C],$\{$attributes$\}$);}{}

\metax{el = brd.create(\'midpoint\',[A,B],$\{$atts$\}$); or [line]}{}
%\metax{el = brd.create(\'midpoint\',[line],$\{$atts$\}$);}{}
\par{\sl -- mirrorpoint:} \hfill rotate point $B$ around point $A$ by $180^\circ$
\metax{el = brd.create(\'mirrorpoint\',[A,B],$\{$atts$\}$);}{}
\par{\sl -- parallel:} \hfill line parallel to line $l$ through point $P$
\metax{el = brd.create(\'parallel\',[l,P],$\{$atts$\}$); or [P,l]}{}
%\metax{el = brd.create(\'parallel\',[P,l],$\{$atts$\}$);}{}
\par{\sl -- parallelpoint:} \hfill point D such that $ABCD$ from a parallelogram
\metax{el = brd.create(\'parallelpoint\',[A,B,C],$\{$atts$\}$);}{}
\par{\sl -- perpendicular:} \hfill line perpendicular to line $l$ through point $P$
\metax{el = brd.create(\'perpendicular\',[l,P],$\{$atts$\}$); or [P,l]}{}
%\metax{el = brd.create(\'perpendicular\',[P,l],$\{$atts$\}$);}{}
\par{\sl -- perpendicularpoint:} \hfill  point defining a perpendicular line to line $l$ through point $P$

\metax{el = brd.create(\'perpendicularpoint\',[l,P],$\{$$\}$); or [P,l]}{}
%\metax{el = brd.create(\'perpendicularpoint\',[P,l],$\{$$\}$);}{}

\par{\sl -- reflection:} \hfill reflection of point $P$ over the line $l$. 
Superseded by transformations

\metax{el = brd.create(\'reflection\',[l,P],$\{$atts$\}$); or [P,l]}{}
%\metax{el = brd.create(\'reflection\',[P,l],$\{$atts$\}$);}{}
\par{\sl -- sector:} \hfill circle sector defined by 3 points \hfill ???
\metax{el = brd.create(\'sector\',[A,B,C],$\{$atts$\}$);}{}

\par{\sl -- semi circle:} \hfill defined by 2 points $p_1$ and $p_2$.
\metax{brd.create(\'semicircle\',[p1,p2],$\{atts\}$);}{}
...
\metax{brd.create(\'intersection\',[o1,o2,n],$\{atts\}$);}{}

\section{Transform}
Affine transformation of objects.\par
\metax{t = brd.create(\'transform\',[data,base],$\{$type:\'type\'$\}$);}{}
...
\metax{p2 = brd.create('point',[p1,t],$\{$fixed:true$\}$);}{}
...
\metax{brd.angle(A,B,C)}{angle $ABC$}
\metax{brd.cosh(x), board.sinh(x)}{}
\metax{brd.pow(a,b)}{$a^b$}
\metax{brd.D(f,x)}{compute ${d\over dx}f$ numerically}
\metax{brd.I([a,b],f)}{compute $\int_a^b f(x)dx$ numerically}
\metax{brd.root(f,x)}{root of the function $f$.}
\metax{}{Uses Newton method with start value $x$}
\metax{brd.factorial(n)}{computes $n!=1\cdot 2\cdot 3\cdots n$}
\metax{brd.binomial(n,k)}{computes ${n\choose k}$}
\metax{brd.distance(arr1,arr2)}{Euclidean distance}
\metax{brd.lagrangePolynomial([p1,p2,...])}{}
\metax{}{returns a polynomial through the given points}
\metax{brd.neville([p1,p2,...])}{polynomial curve interpolation}
\metax{c = JXG.Math.Numerics.bezier([p1,p2,...])}{\hfill Bezier curve}
\metax{}{$p_2,p_3,p_5,p_6,\ldots$ are control points. \kbd{brd.create('curve',c);}}
...
\metax{}{Regression pol. of deg. $n$: \kbd{brd.create('functiongraph',f);}}
\metax{brd.riemannsum(f,n,type,start,end)}{Area of Riemann}
\metax{}{sum, see {\sl Curves}}

{-- Intersection of objects:}\par
\metax{brd.intersection(el1,el2,i,j)}{intersection of the elements}
